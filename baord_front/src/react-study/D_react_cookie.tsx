...
/*
! react-cookie
:React에서 쿠키를 쉽게 관리할 수 있돌고 도와주는 라이브러리
npm i react-cookie
...
react-cookie는 useCookies 훅을 통해
  ...
  const [cookies, setCookie, removeCookie]
    = useCookieses(['쿠키이름']); -배열로 인자값을 전달

  cf) useCookies 훅에 전달되는 배열(인자값)
      : 배열로써 관리하고자 하는 쿠키의 이름을 전달
      : 사용자가 현태 컴포넌트에서 접근하려는 쿠키 이름을 
      지정하는 역할
      - 쿠키에 대한 접근: 'cookies.쿠키임름
*/
...
export default function D_react_cookie() {
  ...
  - 특정 날짜오 시간 지정하여 만료되도로 설정
  maxAge : 쿠키의 유효 시간을 초 단위로 설정
  - 현태 시간부터 지정한 시간 동안 쿠키가 유지
    (생성된 지점으로 부터 시간을 기준으로 유효시간 설정)
  secure : true로 설정 시 HTTPS에서만 쿠키가 전송
  samSite : 쿠키가 전송될 조건을 제한
  - 'strict' 속성을 사용하여
